...
public class TestCloseSale {
    ...
    private void closeSale(String saleId) throws IOException {
        ...
        HtmlPage resultPage = form.getInputByValue("Close Sale").click();
        ...
        for (int i = 1; i < salesTable.getRowCount(); i++) {
        	if (salesTable.getRow(i).getCell(0).asText().equals(saleId)) {
        		assertEquals("C", salesTable.getRow(i).getCell(3).asText());
        		exists = true;
        	}
        }
